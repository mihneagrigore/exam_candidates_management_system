...
function utf8string($str=false, $start=false, $len=false) { # strlen+substr++ combo for UTF-8
  global $CaseConversions;
  static $lower;
  ...
  $start = intval($start);
  if (!is_numeric($len)) return implode('', array_slice($letters, $start));
  return implode('', array_slice($letters, $start, intval($len)));
}
