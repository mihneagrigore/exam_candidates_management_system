...
if (IsEnabled($EnableMergeLastMinorEdit, 0)>=1 && @$Author && PageExists($pagename)) {
  $sameauthor = PageVar($pagename, '$LastModifiedBy') == $Author;
  ...
  if ($sameauthor && $samehost && ($EnableMergeLastMinorEdit < 120 || $ltime>$Now)) {
    $InputTags['e_minorcheckbox']['label'] .= XLHSC(' (merge with previous edit)');
    SDV($InputTags['e_minorcheckbox']['title'], 
      XLHSC('Merge the new edit with the last one, if by the same author, and reuse the change summary if left empty'));
  }
  unset($ltime, $sameauthor, $samehost);
}
